...
class ShortTruthTableImporter extends PuzzleImporter {
    ...
    private void setGivenCells(
        ...
        for (int i = 0; i < cellData.getLength(); i++) {
            ...
            ShortTruthTableCell cell =
                    (ShortTruthTableCell)
                            puzzle.getFactory().importCell(cellData.item(i), sttBoard);
    ...
    public void initializeBoard(Node node) throws InvalidFileFormatException {
        ...
        try {
            ...
            if (!node.getNodeName().equalsIgnoreCase("board")) {
                throw new InvalidFileFormatException(
                        "short truth table Importer: cannot find board puzzleElement");
            }
            Element boardElement = (Element) node;
            if (boardElement.getElementsByTagName("data").getLength() == 0) {
                throw new InvalidFileFormatException(
                        "short truth table Importer: no statements found for board");
            }
            ...
            int maxStatementLength =
                    parseAllStatementsAndCells(statementData, allCells, statements);
            ...
            if (boardElement.getElementsByTagName("goal").getLength() != 0) {
                ...
                Goal goal = puzzle.getFactory().importGoal(goalElement, sttBoard);
            ...
            throw new InvalidFileFormatException(
                    "short truth table Importer: unknown value where integer expected");
        }
    }
