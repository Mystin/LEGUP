        ...
        try {
            if (!node.getNodeName().equalsIgnoreCase("board")) {
                throw new InvalidFileFormatException(
                        "TreeTent Importer: cannot find board puzzleElement");
            }
            Element boardElement = (Element) node;
            if (boardElement.getElementsByTagName("cells").getLength() == 0) {
                throw new InvalidFileFormatException(
                        "TreeTent Importer: no puzzleElement found for board");
            }
            ...
            for (int i = 0; i < elementDataList.getLength(); i++) {
                TreeTentCell cell =
                        (TreeTentCell)
                                puzzle.getFactory()
                                        .importCell(elementDataList.item(i), treeTentBoard);
            ...
            if (!axis1.hasAttribute("side") || !axis1.hasAttribute("side")) {
                throw new InvalidFileFormatException(
                        "TreeTent Importer: side attribute of axis not specified");
            }
            ...
            if (side1.equalsIgnoreCase(side2)
                ...
                throw new InvalidFileFormatException(
                        "TreeTent Importer: axes must be different and be {east | south}");
            }
            NodeList eastClues =
            ...
            if (eastClues.getLength() != treeTentBoard.getHeight()
                    || southClues.getLength() != treeTentBoard.getWidth()) {
                throw new InvalidFileFormatException(
                        "TreeTent Importer: there must be same number of clues as the dimension of"
                                + " the board");
            }
            ...
            for (int i = 0; i < eastClues.getLength(); i++) {
                ...
                if (index - 1 < 0 || index - 1 > treeTentBoard.getHeight()) {
                    throw new InvalidFileFormatException(
                            "TreeTent Importer: clue index out of bounds");
                }
                ...
                treeTentBoard
                        .getRowClues()
                        .set(index - 1, new TreeTentClue(value, index, TreeTentType.CLUE_EAST));
            }
            ...
            for (int i = 0; i < southClues.getLength(); i++) {
                ...
                treeTentBoard
                        .getColClues()
                        .set(index - 1, new TreeTentClue(value, index, TreeTentType.CLUE_SOUTH));
            }
                ...
                for (int i = 0; i < linesList.getLength(); i++) {
                    treeTentBoard
                            ...
                            .add(
                                    (TreeTentLine)
                                            puzzle.getFactory()
                                                    .importCell(linesList.item(i), treeTentBoard));
                }
            }
